+++
title = "6260 notes: setlists,member,subset_union,subset_trans"
date = 2026-08-31
categories = ["6260"]
...
```angular2html
case Cons(hd, tl) => member_subset(x, tl, B); 这一步逐帧展开：
```

你手里有什么、欠什么、每一步谁付账，全部摊开。

**第 0 帧：开工时的账本。**

- 手里的资产（requires）：`subset(A, B)` 为真。
- 欠的债（ensures）：`member(x, A) ==> member(x, B)`。

两条都是关于抽象的 `A` 的命题。**`subset` 和 `member` 都是 match A 定义的，A 的构造器不明就一步也展不开**——这就是体内第一行必须 `match A` 的全部理由：案例拆分唯一的作用是让构造器现形。

**第 1 帧：case Nil，A 现形为 Nil。**

把 `Nil` 代进欠的债：`member(x, Nil)`。按定义展开，`match Nil case Nil => false`，得 `false`。债变成 `false ==> member(x, B)`。前件为假的蕴含恒真——债自动作废。所以这一支什么都不用写，空着就是完整证明。

**第 2 帧：case Cons(hd, tl)，两边同时展开。**

先展手里的资产。`subset(Cons(hd, tl), B)` 按定义走 Cons 分支：

```
member(hd, B) && subset(tl, B)
```

一份资产裂成两份：**① `member(hd, B)`；② `subset(tl, B)`。**

再展欠的债的前件。`member(x, Cons(hd, tl))` 按定义走 Cons 分支：

```
x == hd || member(x, tl)
```

债变成：`(x == hd || member(x, tl)) ==> member(x, B)`。前件是个"或"，两条路都得堵死。

**第 3 帧：路一，x == hd。**

那么 `member(x, B)` 就是 `member(hd, B)`——**资产①原封不动就是答案**。不需要写任何东西，Dafny 自己替换即得。

**第 4 帧：路二，member(x, tl)。**

现在欠的是 `member(x, tl) ==> member(x, B)`。盯着看：这**正是本 lemma 自己的陈述，只是 A 换成了 tl**。lemma 的参数就是全称量词——这条 lemma 说的是"对所有 A"，那么对 `tl` 当然也该成立。于是写下：

```dafny
member_subset(x, tl, B);
```

这一行调用发生两件事，方向相反：

- **你付账**：调用处必须先满足它的 requires，即 `subset(tl, B)`——用资产②支付，恰好够。
- **它付你**：调用完成后，它的 ensures 成为你手里的新事实：`member(x, tl) ==> member(x, B)`。路二的债，正好用这张新票据结清。

这就是"归纳假设"在 Dafny 里的物理形态：**递归调用一条 lemma，requires 是你先垫的钱，ensures 是它还你的货**。合法性由 `decreases` 保障——`tl` 是 `A` 的严格子结构，账不会循环。

**第 5 帧：为什么不用写 if。**

两条路的票据此刻同时躺在账本上：路一靠资产①，路二靠第 4 帧的调用结果。"或"的两个分支各有着落，`||` 的拆分是纯逻辑推演，SMT 求解器自动完成。你只需保证**每条路的票据都已备齐**——那行递归调用存在的意义，就是把路二的票据提前放进账本。

一句话收拢：match 让定义展得开，展开让资产裂成两份，一份直付路一，一份用来垫付归纳假设的前置、换回路二的票据。
